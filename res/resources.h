#ifndef _RES_RESOURCES_H_
#define _RES_RESOURCES_H_

extern const u8 sfx_enter[11264];
extern const u8 sfx_attack[3328];
extern const u8 sfx_eat[39168];
extern const u8 sfx_jump[4352];
extern const u8 sfx_walk[3840];
extern const u8 main_menu_bgm[3072];
extern const u8 gameplay_bgm[3840];
extern const u8 ending_bgm[1280];
extern const Image splash;
extern const Image title;
extern const TileSet game_tileset;
extern const Palette game_pal;
extern const MapDefinition gamemap;
extern const MapDefinition foremap;
extern const SpriteDefinition player_sprite;
extern const SpriteDefinition tapioca_sprite;
extern const Image lose_screen;
extern const Image win_screen;

#endif // _RES_RESOURCES_H_
